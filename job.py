...
class Job(object):
    ...
    def add_cmd(self, cmd):
        if self.cmds.empty():
            log_str =  " 2>&1 | tee %s/%s.txt" % (Job.log_folder, self.id)
        else:
...
class Qemu(Job):
    def __init__(self):
        ...
        super().__init__(1, "/home/joakim.bech/devel/optee_projects/qemu")
        self.jobtype = "qemu"
        ...
        self.add_cmd("ln -s %s/toolchains %s/toolchains" % (Job.reference_folder, self.workspace))
...
class Rpi3(Job):
    def __init__(self):
        ...
        super().__init__(2, "/home/joakim.bech/devel/optee_projects/rpi3")
        self.jobtype = "rpi3"
        # FIXME: All this could go into "config-files" instead of hardcoding it
        # here.
        self.add_cmd("repo forall -c 'git checkout -f && git clean -xdf'")
...
class Fvp(Job):
    def __init__(self):
        ...
        super().__init__(3, "/home/joakim.bech/devel/optee_projects/fvp")
        self.jobtype = "fvp"
        # FIXME: All this could go into "config-files" instead of hardcoding it
        # here.
        self.add_cmd("repo forall -c 'git checkout -f && git clean -xdf'")
